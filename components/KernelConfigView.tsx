...
export const KernelConfigView: React.FC<Props> = ({ config, domain, overrideShort, overrideMid, overrideLong }) => {
  // Merge overrides into config for display
  const effectiveConfig = useMemo(() => {
    const next = { ...config };
    const interval = next.sample_interval_sec || 30;

    if (overrideShort) {
      next.drift = { ...next.drift, short_window_samples: overrideShort };
      next.oscillation = {
        ...next.oscillation,
        window_samples: overrideShort,
        window_seconds: overrideShort * interval
      };
    }

    if (overrideLong) {
      next.drift = { ...next.drift, long_window_samples: overrideLong };
    }

    if (overrideMid) {
      next.stability = { ...next.stability, window_samples: overrideMid };
      next.boundary = { ...next.boundary, window_samples: overrideMid };
    }

    return next;
  }, [config, overrideShort, overrideMid, overrideLong]);

  return (
    <div className="space-y-4 mb-6">
      <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-widest border-b border-slate-700 pb-2 flex justify-between items-center">
        <span>Kernel Configuration (Domain: {domain})</span>
        <span className="text-xs px-2 py-1 rounded bg-slate-800 text-cyan-400 border border-slate-600">
          Sample Interval: {effectiveConfig.sample_interval_sec || 30}s
        </span>
      </h3>
      <div className="flex flex-wrap gap-4">
        <ConfigSection
          ...
          data={effectiveConfig.drift}
          color="text-blue-400"
        />
        <ConfigSection
          title="Stability"
          icon={<Anchor size={14} />}
          data={effectiveConfig.stability}
          color="text-emerald-400"
        />
        <ConfigSection
          title="Boundary"
          icon={<AlertTriangle size={14} />}
          data={effectiveConfig.boundary}
          color="text-amber-400"
        />
        <ConfigSection
          title="Oscillation"
          icon={<GitCommit size={14} />}
          data={effectiveConfig.oscillation}
