            ...
            {processedData.length > 0 ? (
                ...
                <tr key={row.timestamp} className="hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-4 py-2">
                        {/* 
                          Deterministic drift score calculation based on timestamp for demo stability.
                          In a real app, this would come from the backend. 
                        */}
                        {((row.timestamp % 100) / 100) > 0.6 ? 
                            <span className="text-red-400 font-bold">{(0.7 + (row.timestamp % 25)/100).toFixed(2)}</span> : 
                            <span className="text-emerald-500">{(0.1 + (row.timestamp % 25)/100).toFixed(2)}</span>
                        }
                    </td>
                </tr>
                ))
